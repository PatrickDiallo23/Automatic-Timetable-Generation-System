...
case "$COMMAND" in
  start)
    ...
    echo "    → App:      https://localhost:$(grep NGINX_PORT $ENV_FILE | cut -d= -f2 | tr -d ' ')"
    echo "    → Backend:  https://localhost:8200/actuator/health"
    echo "    → Swagger:  https://localhost:$(grep NGINX_PORT $ENV_FILE | cut -d= -f2 | tr -d ' ')/swagger-ui.html"
